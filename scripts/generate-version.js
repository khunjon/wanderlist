...
function generateVersion() {
  ...
  if (environment === 'production' && process.env.VERCEL_GIT_COMMIT_SHA) {
    ...
    version = gitHash;
    console.log(`✓ Using Vercel git hash: ${gitHash}`);
  }
...
function updateEnvFile(versionInfo) {
  ...
  try {
    ...
    console.log(`✓ Updated .env.local with version info`);
  } catch (error) {
...
`;
  ...
  try {
    ...
    console.log(`✓ Version constants written to: ${constantsPath}`);
  } catch (error) {
    console.error('❌ Failed to write version constants:', error);
    process.exit(1);
  }
}
